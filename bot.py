...
def shipping(update: Update, context: CallbackContext):
    ...
    with open('counters.json') as counters_fp:
        try:
            ...
            first_name1 = context.bot.get_chat_member(chat_id=update.effective_chat.id, user_id=user_id_shipped1).user.first_name
            last_name1 = context.bot.get_chat_member(chat_id=update.effective_chat.id, user_id=user_id_shipped1).user.last_name
            first_name2 = context.bot.get_chat_member(chat_id=update.effective_chat.id, user_id=user_id_shipped2).user.first_name
            last_name2 = context.bot.get_chat_member(chat_id=update.effective_chat.id, user_id=user_id_shipped2).user.last_name
            logging.info(shippable)
            text = 'La coppia del giorno:\n\n' if shippable else 'La coppia del giorno è già stata scelta:\n\n'
            text += f'{first_name1} {last_name1} ' if last_name1 else f'{first_name1} '
            text += f'+ {first_name2} {last_name2} ' if last_name2 else f'+ {first_name2} = ❤\n'
